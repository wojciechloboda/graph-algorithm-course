...
def create_graph(N, M, K, base, wages, eq_cost):
    ...
    bases_graph = [[None for _ in base[i]] for i in range(N)]
    ...
    for i in range(0, N):
        for j in range(len(base[i])):
            G.append([])
            if j == 0:
                bases_graph[i][j] = current
                G[1].append((current, len(base[i]), 0)) 
            else:
                bases_graph[i][j] = current
                G[bases_graph[i][j - 1]].append((current, K, 0))
            current += 1
    for i in range(0, N):
        G.append([])
        ...
        for j in range(len(base[i])):
            if j > 0:
                G[bases_graph[i][j]].append((current, 1, base[i][j] - base[i][j - 1]))
            else:
                G[bases_graph[i][j]].append((current, 1, base[i][j]))
        current += 1
