from dimacs import loadDirectedWeightedGraph
import os
from queue import Queue

INF = float('inf')

def BFS(G, G_list, s, parent, L):
    Q = Queue()
    Q.put(s)
    while not Q.empty():
        v = Q.get()
        for u in G_list[v]:
            if G[v][u] > 0 and parent[u] == None:
                parent[u] = v
                Q.put(u)

def DFS(G, G_list, v, parent, L):
    for u in G_list[v]:
        if G[v][u] > 0 and parent[u] == None:
            parent[u] = v
            DFS(G, G_list, u, parent, L)

def ford_fulkerson(L, V, s, t):
    G = [[0 for _ in range(L + 1)] for _ in range(L + 1)]
    orig = [[False for _ in range(L + 1)] for _ in range(L + 1)]

    G_list = [[] for _ in range(L + 1)]

    for v, u, weight in V:
        G[v][u] = weight
        G_list[v].append(u)
        G_list[u].append(v) 
        orig[v][u] = True

    while True:
        parent = [None for _ in range(L + 1)]
        parent[s] = s

        DFS(G, G_list, s, parent, L)
        if parent[t] == None:
            res = 0
            for v in range(L + 1):
                if orig[s][v]:
                    res += G[v][s]
            return res 

        current = t
        current_min = INF
        while parent[current] != current:
            current_min = min(current_min, G[parent[current]][current])
            current = parent[current]
        
        current = t
        while parent[current] != current:
            G[current][parent[current]] += current_min
            G[parent[current]][current] -= current_min
            current = parent[current]


def edmonds_karp(L, V, s, t):
    G = [[0 for _ in range(L + 1)] for _ in range(L + 1)]
    orig = [[False for _ in range(L + 1)] for _ in range(L + 1)]

    G_list = [[] for _ in range(L + 1)]

    for v, u, weight in V:
        G_list[v].append(u)
        G_list[u].append(v) 
        G[v][u] = weight
        orig[v][u] = True


    while True:
        parent = [None for _ in range(L + 1)]
        parent[s] = s

        BFS(G, G_list, s, parent, L)
        if parent[t] == None:
            res = 0
            for v in range(L + 1):
                if orig[s][v]:
                    res += G[v][s]
            return res

        current = t
        current_min = INF
        while parent[current] != current:
            current_min = min(current_min, G[parent[current]][current])
            current = parent[current]
        
        current = t
        while parent[current] != current:
            G[current][parent[current]] += current_min
            G[parent[current]][current] -= current_min
            current = parent[current]


V, L = loadDirectedWeightedGraph('graphs/rand100_500')

#print('simple', "test: ", ford_fulkerson(V, L, 1, V))

#sys.setrecursionlimit(1000000000)
for file in os.listdir("graphs"):
    with open('graphs/' + file, 'r') as f:
        solution = int(f.readline().split()[3])
        V, L = loadDirectedWeightedGraph('graphs/' + file)
        print(file, "test: ", solution == edmonds_karp(V, L, 1, V))
